...
const getVelas = async () => { 
  // CORREÇÃO: Filtrando pela categoria_id = 1 (Velas)
  const { data, error } = await supabase
    .from('core_produto')
    .select('*')
    .eq('categoria_id', 1) 
    .limit(4) // <<< ESTA LINHA LIMITA A BUSCA A NO MÁXIMO 8 ITENS
...
.velas-container { padding: 60px 80px; background-color: #fff; display: flex; flex-direction: column; align-items: center; }
...
.box-vela { background-color: #8CB3C6; padding: 24px; display: flex; flex-direction: column; align-items: center; }
.container-img img { width: 100%; height: auto; }
...
.btn-adicionar { background-color: #fff; color: #8CB3C6; font-weight: bold; border: none; padding: 10px 18px; font-size: 16px; cursor: pointer; }
</style>